...
func (l *Loop) processLLMRequest(ctx context.Context) error {
	for {
		...
		seenTransformations := make(map[llm.InputTransformation]bool)
		sendWithRetry := func(req *llm.Request) (*llm.Response, error) {
			...
			for attempt := 1; attempt <= maxRetries; attempt++ {
				...
				if err == nil {
					l.recordInputTransformationWarnings(llmCtx, resp, seenTransformations)
					return resp, nil
				}
...
func (l *Loop) resolvePausedTurn(
	...
	transformations := append([]llm.InputTransformation(nil), resp.InputTransformations...)
	...
	for i := 0; resp.StopReason == llm.StopReasonPause; i++ {
		...
		for _, entry := range next.InputTransformations {
	...
	resolved.InputTransformations = transformations
...
// Record diagnostics separately from history so they are visible, but never sent
// to the model. Record each successful pause leg too, even if resuming fails.
func (l *Loop) recordInputTransformationWarnings(ctx context.Context, resp *llm.Response, seen map[llm.InputTransformation]bool) {
	if resp == nil {
		return
	}
	var reasons []string
	counts := make(map[string]int)
	for _, entry := range resp.InputTransformations {
		if seen[entry] {
			continue
		}
		seen[entry] = true
		l.logger.WarnContext(ctx, "LLM input transformation", "response_id", resp.ID,
			"model", resp.Model, "type", entry.Type, "path", entry.Path, "reason", entry.Reason)
		if entry.Type != "thinking_dropped" || l.recordWarning == nil {
			continue
		}
		if counts[entry.Reason] == 0 {
			reasons = append(reasons, entry.Reason)
		}
		counts[entry.Reason]++
	}
	for _, reason := range reasons {
		why := "the provider did not supply a recognized reason"
		switch reason {
		case "prefix_binding_mismatch":
			why = "the preceding conversation no longer matches the context these blocks were bound to"
		case "model_binding_mismatch":
			why = "the serving model cannot read thinking generated by the earlier model"
		}
		msg := fmt.Sprintf("The provider dropped %d historical thinking block(s): %s. Continuing without those blocks.", counts[reason], why)
		if err := l.recordWarning(ctx, msg); err != nil {
			// As with retry warnings, a diagnostic write failure must not discard
			// a successful model response (which may contain pending tool calls).
			l.logger.Error("failed to record input transformation warning", "error", err)
		}
	}
}
