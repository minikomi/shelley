...
// Loop manages conversation turns with an LLM including tool execution and message recording.
// Go keeps the loop alive between turns until its context is canceled.
type Loop struct {
	llm              llm.Service
	tools            []*llm.Tool
	recordMessage    MessageRecordFunc
	...
	for {
		...
		sendWithRetry := func(req *llm.Request) (*llm.Response, error) {
			...
			for attempt := 1; attempt <= maxRetries; attempt++ {
				...
				if err == nil {
					l.logInputTransformations(llmCtx, resp, seenTransformations)
					return resp, nil
				}
...
// Log safe diagnostics only: no user-visible banners or changes to model history.
// Include each successful pause leg too, even if resuming fails.
func (l *Loop) logInputTransformations(ctx context.Context, resp *llm.Response, seen map[llm.InputTransformation]bool) {
	if resp == nil {
		return
	}
