...
run("conversationMatchesTags mirrors the list filter", () => {
  assert(conversationMatchesTags(corpus[0], ["infra", "urgent"]), "a matches both");
  assert(!conversationMatchesTags(corpus[1], ["infra", "urgent"]), "b lacks urgent");
  assert(conversationMatchesTags(corpus[4], []), "untagged matches empty selection");
});
...
run("presentTags reports the folded vocabulary", () => {
  const present = presentTags(corpus);
  assert(present.size === 3, `size ${present.size}`);
  assert(present.has("infra") && present.has("urgent") && present.has("docs"), "all three");
  assert(presentTags([conv("a", null)]).size === 0, "no tags anywhere");
});
