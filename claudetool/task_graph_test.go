...
func TestTaskGraphToolDescriptionRequiresGraphBeforeMultiScopeWork(t *testing.T) {
	...
	for _, required := range []string{
		...
		"name the unanswered questions",
		"Close research once authoritative sources answer them",
		"after the graph finishes unless validation exposes",
		...
		"exclude optional",
		"Quote fragile contract values",
		"exact IDs, severity labels",
		"routes, selectors, and wire values",
		"instead of merely saying",
		"Focused tests should assert those quoted values",
		"reconstructing them from memory",
		"Stop when the focused validation passes",
		"operational purpose, not just a deliverable",
		"shortens the request's critical path",
		"what decisions it owns",
		"trust without inspecting sibling work",
		"exactly when it should stop",
		"one batched inspection",
		"one primary work pass",
		"at most one repair pass",
		"concrete validation failures",
		"whole request's critical path",
		"local polish",
		"Luna with",
		...
		"non-overlapping",
		"parent's work as another lane",
		"cohesive objective, validation, and stop condition",
		"leftovers bucket",
		"expected inspection, work",
		"expected slowest child",
		"rebalance the scopes",
		"completed direct",
		"handoff context",
		...
		"creative tail: absent",
		"no new research, implementation, or",
