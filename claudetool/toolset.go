...
type ToolSetConfig struct {
	...
	// PatchOpenAIRawEnabled lets OpenAI Responses services override the selected
	// full/simple schema with the native apply_patch tool.
...
func NewToolSet(ctx context.Context, cfg ToolSetConfig) *ToolSet {
	...
	if cfg.LLMProvider != nil && cfg.ModelID != "" {
		if svc, err := cfg.LLMProvider.GetService(cfg.ModelID); err == nil {
			...
			if cfg.PatchOpenAIRawEnabled != nil && cfg.PatchOpenAIRawEnabled() {
				if profile := llm.PatchProfile(svc); profile != llm.PatchProfileFlat {
